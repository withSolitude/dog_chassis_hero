...
void Class_Referee::UART_Send_Interaction_UI_Graphic_1(Struct_Referee_Data_Interaction_Graphic_Config *Graphic_1)
{
    Struct_Referee_UART_Data *tmp_buffer = (Struct_Referee_UART_Data *)Tx_Buffer;
...
/**
 * @brief 数据处理过程（通用流式解析版）
 *
 * 适配 2026 通信协议帧格式：
 * SOF(0xA5) + DataLength(2) + Seq(1) + CRC8(1) + CmdID(2) + Data(n) + CRC16(2)
 *
 * 特点：
 * 1) 支持半包 / 粘包（DMA空闲中断常见）
 * 2) 逐字节滑窗找帧头，CRC8/CRC16 完整校验
 * 3) 不依赖具体机器人类型（英雄/步兵/哨兵/工程都通用）
 * 4) 对未知 CmdID 自动跳过，不会卡死
 *
 */
#include <cstring>   // memcpy/memmove/memset

void Class_Referee::Data_Process(const uint8_t *rx, uint16_t len)
{
    if (rx == nullptr || len == 0)
    {
        return;
    ...
    // 流式拼包缓存（单实例工程里一般只有一个裁判系统对象，使用静态缓存即可）
    static uint8_t stream_buf[512];
    static uint16_t stream_len = 0;

    // ---------- 1) 拼接本次收到的数据 ----------
    if ((uint16_t)(stream_len + len) > (uint16_t)sizeof(stream_buf))
    {
        // 缓存溢出时直接丢弃旧数据，避免越界
        stream_len = 0;
    }

    memcpy(&stream_buf[stream_len], rx, len);
    stream_len = (uint16_t)(stream_len + len);

    // ---------- 2) 循环解析缓存中的完整帧 ----------
    while (stream_len >= 5) // 至少要够 SOF+LEN+SEQ+CRC8
    {
        // 2.1 找帧头 0xA5
        int sof_idx = -1;
        for (uint16_t i = 0; i < stream_len; i++)
        {
            if (stream_buf[i] == 0xA5)
            {
                sof_idx = (int)i;
                break;
            }
        ...
        if (sof_idx > 0)
        {
            memmove(stream_buf, &stream_buf[sof_idx], stream_len - (uint16_t)sof_idx);
            stream_len = (uint16_t)(stream_len - (uint16_t)sof_idx);
        }
        ...
        // 2.2 校验帧头 CRC8（前4字节，CRC在第5字节）
        if (Verify_CRC_8((uint8_t *)stream_buf, 4) != stream_buf[4])
        {
            // 不是合法帧头，丢1字节继续找
        ...
        // 2.3 读取长度字段（小端）
        uint16_t data_len = (uint16_t)(stream_buf[1] | (stream_buf[2] << 8));

        // 长度保护：协议载荷不会超过本结构体Data[121]
        if (data_len > 121)
        {
            memmove(stream_buf, &stream_buf[1], stream_len - 1);
            stream_len--;
        ...
        const uint16_t frame_len = (uint16_t)(7 + data_len + 2); // 头5 + cmd2 + data + crc16
        ...
        // 2.5 CRC16 校验（小端）
        uint16_t crc16_calc = Verify_CRC_16((uint8_t *)stream_buf, frame_len - 2);
        uint16_t crc16_recv = (uint16_t)(stream_buf[frame_len - 2] | (stream_buf[frame_len - 1] << 8));
        ...
        // ---------- 3) 一帧有效，按 CmdID 解包 ----------
        uint16_t cmd_id = (uint16_t)(stream_buf[5] | (stream_buf[6] << 8));
        const uint8_t *payload = &stream_buf[7];

        // 安全拷贝：按实际 payload 长度截断复制
        #define COPY_PAYLOAD_SAFE(dst_obj)                                                       \
            do                                                                                   \
            {                                                                                    \
                memset(&(dst_obj), 0, sizeof(dst_obj));                                          \
                uint16_t __cp_len = data_len;                                                    \
                if (__cp_len > sizeof(dst_obj))                                                  \
                {                                                                                \
                    __cp_len = (uint16_t)sizeof(dst_obj);                                        \
                }                                                                                \
                memcpy(&(dst_obj), payload, __cp_len);                                           \
            } while (0)

        switch ((Enum_Referee_Command_ID)cmd_id)
        {
        case (Referee_Command_ID_GAME_STATUS):
            COPY_PAYLOAD_SAFE(Game_Status);
            break;
        case (Referee_Command_ID_GAME_RESULT):
            COPY_PAYLOAD_SAFE(Game_Result);
            break;
        case (Referee_Command_ID_GAME_ROBOT_HP):
            COPY_PAYLOAD_SAFE(Game_Robot_HP);
            break;

        case (Referee_Command_ID_EVENT_SELF_DATA):
            COPY_PAYLOAD_SAFE(Event_Self_Data);
            break;
        case (Referee_Command_ID_EVENT_SELF_SUPPLY):
            COPY_PAYLOAD_SAFE(Event_Self_Supply);
            break;
        case (Referee_Command_ID_EVENT_SELF_REFEREE_WARNING):

            COPY_PAYLOAD_SAFE(Event_Referee_Warning);
            break;
        case (Referee_Command_ID_EVENT_SELF_DART_STATUS):
            COPY_PAYLOAD_SAFE(Event_Dart_Status);
            break;

        case (Referee_Command_ID_ROBOT_STATUS):
            COPY_PAYLOAD_SAFE(Robot_Status);
            break;
        case (Referee_Command_ID_ROBOT_POWER_HEAT):
            COPY_PAYLOAD_SAFE(Robot_Power_Heat);
            break;
        case (Referee_Command_ID_ROBOT_POSITION):
            COPY_PAYLOAD_SAFE(Robot_Position);
            break;
        case (Referee_Command_ID_ROBOT_BUFF):
            COPY_PAYLOAD_SAFE(Robot_Buff);
            break;
        case (Referee_Command_ID_ROBOT_AERIAL_STATUS):
            COPY_PAYLOAD_SAFE(Robot_Aerial_Status);
            break;
        case (Referee_Command_ID_ROBOT_DAMAGE):
            COPY_PAYLOAD_SAFE(Robot_Damage);
            break;
        case (Referee_Command_ID_ROBOT_BOOSTER):
            COPY_PAYLOAD_SAFE(Robot_Booster);
            break;
        case (Referee_Command_ID_ROBOT_REMAINING_AMMO):
            COPY_PAYLOAD_SAFE(Robot_Remaining_Ammo);
            break;
        case (Referee_Command_ID_ROBOT_RFID):
            COPY_PAYLOAD_SAFE(Robot_RFID);
            break;
        case (Referee_Command_ID_ROBOT_DART_COMMAND):
            COPY_PAYLOAD_SAFE(Robot_Dart_Command);
            break;
        case (Referee_Command_ID_ROBOT_SENTRY_LOCATION):
            COPY_PAYLOAD_SAFE(Robot_Sentry_Location);
            break;
        case (Referee_Command_ID_ROBOT_RADAR_MARK):
            COPY_PAYLOAD_SAFE(Robot_Radar_Mark);
            break;
        case (Referee_Command_ID_ROBOT_SENTRY_DECISION):
            COPY_PAYLOAD_SAFE(Robot_Sentry_Decision);
            break;
        case (Referee_Command_ID_ROBOT_RADAR_DECISION):
            COPY_PAYLOAD_SAFE(Robot_Radar_Decision);
            break;
