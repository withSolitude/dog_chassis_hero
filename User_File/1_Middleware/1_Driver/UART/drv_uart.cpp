...
{
    if (Rx_Buffer_Length == 0 || Rx_Buffer_Length > UART_BUFFER_SIZE)
    {
        Rx_Buffer_Length = UART_BUFFER_SIZE;
    }

    Struct_UART_Manage_Object *obj = nullptr;

    if (huart->Instance == USART1)      obj = &UART1_Manage_Object;
    else if (huart->Instance == USART2) obj = &UART2_Manage_Object;
    else if (huart->Instance == USART3) obj = &UART3_Manage_Object;
    else if (huart->Instance == UART4)  obj = &UART4_Manage_Object;
    else if (huart->Instance == UART5)  obj = &UART5_Manage_Object;
    else if (huart->Instance == USART6) obj = &UART6_Manage_Object;
    else if (huart->Instance == UART7)  obj = &UART7_Manage_Object;
    else if (huart->Instance == UART8)  obj = &UART8_Manage_Object;
    else if (huart->Instance == UART9)  obj = &UART9_Manage_Object;
    else if (huart->Instance == USART10) obj = &UART10_Manage_Object;
    ...
    obj->Rx_Buffer_Ready = nullptr;
    obj->Rx_Time_Stamp = 0;

    HAL_UARTEx_ReceiveToIdle_DMA(huart, obj->Rx_Buffer_Active, obj->Rx_Buffer_Length);
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    if (!init_finished)
    {
        ...
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART1_Manage_Object.Rx_Buffer_Active, UART1_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == USART2)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART2_Manage_Object.Rx_Buffer_Active, UART2_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == USART3)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART3_Manage_Object.Rx_Buffer_Active, UART3_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == UART4)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART4_Manage_Object.Rx_Buffer_Active, UART4_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == UART5)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART5_Manage_Object.Rx_Buffer_Active, UART5_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == USART6)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART6_Manage_Object.Rx_Buffer_Active, UART6_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == UART7)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART7_Manage_Object.Rx_Buffer_Active, UART7_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == UART8)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART8_Manage_Object.Rx_Buffer_Active, UART8_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == UART9)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART9_Manage_Object.Rx_Buffer_Active, UART9_Manage_Object.Rx_Buffer_Length);
        }
        else if (huart->Instance == USART10)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(huart, UART10_Manage_Object.Rx_Buffer_Active, UART10_Manage_Object.Rx_Buffer_Length);
        }
        return;
    }
